#!/usr/bin/env bash
# Gemma 4 31B IT Uncensored Heretic Q8_0 — single DGX Spark controller (llama.cpp)
# Runtime: llama.cpp built from source, SM121a-real (GB10 Blackwell)
# Vision: gemma-4-31B-it-mmproj-BF16.gguf; auto-fallback to text-only on known CUDA/mmproj failures
# Thinking: configurable with --thinking on|off via Gemma 4 chat-template kwargs
set -Eeuo pipefail

SCRIPT_VERSION="${SCRIPT_VERSION:-3.1.0}"
MODEL_LABEL="${MODEL_LABEL:-Gemma-4-31B Uncensored Heretic (Q8_0 GGUF)}"
RUNTIME_LABEL="${RUNTIME_LABEL:-llama.cpp}"
MODEL_FEATURES="${MODEL_FEATURES:-vision · thinking · tools · uncensored}"

# ─── Model ────────────────────────────────────────────────────────────────────
HF_REPO="${HF_REPO:-Andu-bit/gemma-4-31B-it-uncensored-heretic-GGUF}"
MODEL_REVISION="${MODEL_REVISION:-main}"

# Q8_0 is the requested default; other repository quantizations remain selectable
MODEL_QUANT="${MODEL_QUANT:-Q8_0}"
MODEL_FILE="${MODEL_FILE:-gemma-4-31B-it-uncensored-heretic-${MODEL_QUANT}.gguf}"

# mmproj (vision encoder projector) — required for image/video inputs
MMPROJ_FILE="${MMPROJ_FILE:-gemma-4-31B-it-mmproj-BF16.gguf}"

SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-gemma-4-31b-it-uncensored-heretic-q8_0}"
...
MODEL_DIR="${MODEL_DIR:-${USER_HOME}/models/gemma-4-31b-it-uncensored-heretic}"
...
VISION_MODE="${VISION_MODE:-auto}"   # auto, on, off
STARTUP_TIMEOUT="${STARTUP_TIMEOUT:-300}"

# ─── Approximate file sizes for verification (bytes; update after first download)
MODEL_SIZE_MIN_BYTES="32000000000"   # Q8_0 exact size is 32,635,669,984 bytes
MMPROJ_SIZE_MIN_BYTES="1200000000"  # exact size is 1,200,726,208 bytes

# Exact SHA-256 values from the Hugging Face Xet/LFS pointers
MODEL_SHA256="${MODEL_SHA256:-1d3e0b4b9f5474109711293d2c10d47183ea9c2498b993169fc3997d766007f1}"
MMPROJ_SHA256="${MMPROJ_SHA256:-21487ff26d08f7ddd1d654d3bbfc1ae1020aab3119f5bf654742ce4697732e4e}"

# ─── Server PID file ──────────────────────────────────────────────────────────
PID_FILE="/tmp/llama-gemma4-31b-heretic.pid"
...
model_size_min_bytes() {
  case "$MODEL_QUANT" in
    Q3_K_M) echo 15000000000 ;;
    Q3_K_L) echo 16000000000 ;;
    Q4_K_S) echo 17000000000 ;;
    Q4_K_M) echo 18000000000 ;;
    Q5_K_S) echo 20500000000 ;;
    Q5_K_M) echo 21000000000 ;;
    Q6_K)   echo 24500000000 ;;
    Q8_0)   echo 32000000000 ;;
    BF16)   echo 60000000000 ;;
...
parse_options() {
  ...
  while (( $# )); do
    case "$1" in
      ...
      --quant)          MODEL_QUANT="$2"; MODEL_FILE="gemma-4-31B-it-uncensored-heretic-${MODEL_QUANT}.gguf"; shift 2 ;;
      --quant=*)        MODEL_QUANT="${1#*=}"; MODEL_FILE="gemma-4-31B-it-uncensored-heretic-${MODEL_QUANT}.gguf"; shift ;;
  ...
  case "$THINKING" in on|off) ;; *) die "Invalid --thinking value: ${THINKING} (use on or off)" ;; esac
  case "$VISION_MODE" in auto|on|off) ;; *) die "Invalid --vision value: ${VISION_MODE} (use auto, on, or off)" ;; esac
  ...
  export CTX_SIZE API_HOST API_PORT ADVERTISE_IP ADVERTISE_INTERFACE MODEL_QUANT MODEL_FILE PARALLEL_SLOTS FLASH_ATTN SPLIT_MODE CACHE_TYPE_K CACHE_TYPE_V THINKING VISION_MODE STARTUP_TIMEOUT
}
...
download() {
  ...
  case "$subcmd" in
    --list)
      echo "Available files for ${HF_REPO}:"
      echo ""
      echo "  Quant   Approx size   Notes"
      echo "  Q3_K_M  15.3 GB       Smallest listed quant"
      echo "  Q3_K_L  16.6 GB       Better Q3 quality"
      echo "  Q4_K_S  17.8 GB       Smaller Q4"
      echo "  Q4_K_M  18.7 GB       Balanced"
      echo "  Q5_K_S  21.3 GB       Smaller Q5"
      echo "  Q5_K_M  21.8 GB       Good quality"
      echo "  Q6_K    25.2 GB       Excellent quality"
      echo "  Q8_0    32.6 GB       Near-lossless (DEFAULT)"
      echo "  BF16    61.4 GB       Native precision"
      echo ""
      echo "Vision projector:"
      echo "  ${MMPROJ_FILE}  1.2 GB  Required for image/video-frame input"
      echo ""
      echo "Suggested DGX Spark starting point:"
      echo "  Q8_0 + BF16 mmproj + 128K ctx + q8 KV + parallel=1"
      return
      ;;
    --force)
      force_download=1
  ...
  if [[ -z "$hf_python" ]]; then
    ...
    "${HF_VENV}/bin/python" -m pip install --disable-pip-version-check --upgrade pip huggingface_hub hf_xet
    hf_python="${HF_VENV}/bin/python"
  fi
...
verify_files() {
  ...
  if [[ "$MODEL_QUANT" == "Q8_0" ]] && (( msz != 32635669984 )); then
    die "Unexpected Q8_0 size: ${msz} bytes (expected exactly 32635669984)"
  fi
  ...
  if (( psz != 1200726208 )); then
    die "Unexpected mmproj size: ${psz} bytes (expected exactly 1200726208)"
  fi
...
start() {
  ...
  mkdir -p "$MODEL_DIR"
  local thinking_bool=false
  [[ "$THINKING" == "on" ]] && thinking_bool=true
  ...
  launch_server() {
    ...
    if "$srv" -h 2>&1 | grep -F -- "--chat-template-kwargs" >/dev/null; then
      args+=(--chat-template-kwargs "{\"enable_thinking\":${thinking_bool}}")
    else
      log "WARN: this llama-server does not advertise --chat-template-kwargs; THINKING=${THINKING} may not be enforced."
    fi
  ...
  local first_mode="$VISION_MODE"
  [[ "$first_mode" == "auto" ]] && first_mode="on"

  log "Starting llama-server …"
  log "  Model   : ${model_path}"
  log "  Quant   : ${MODEL_QUANT}"
  log "  Vision  : ${first_mode}"
  [[ "$first_mode" == "on" ]] && log "  mmproj  : ${mmproj_path}"
  ...
  local rc=$?
  local failed_pid=""
  [[ -f "$PID_FILE" ]] && failed_pid=$(cat "$PID_FILE")
  [[ -z "$failed_pid" ]] || kill "$failed_pid" 2>/dev/null || true
  rm -f "$PID_FILE"

  if [[ "$VISION_MODE" == "auto" ]] \
      && grep -Eqi 'mmproj|clip_model_loader|mtmd|SIGABRT|vision encoder' "$LOG_FILE"; then
    echo "" >&2
    log "WARN: Gemma 4 vision projector failed on CUDA; retrying text-only."
    log "WARN: use '--vision on start' to require vision and fail instead of falling back."
    launch_server off
    log "Fallback server PID: $(cat "$PID_FILE")"
    if wait_for_health; then
      local adv
      adv=$(detect_advertise_ip)
      echo ""
      echo "  Model    : ${SERVED_MODEL_NAME} (${MODEL_QUANT})"
      echo "  Vision   : off (automatic fallback)"
      echo "  Thinking : ${THINKING}"
      echo "  API      : http://${adv}:${API_PORT}/v1"
      echo "  Context  : ${CTX_SIZE} tokens"
      return
  ...
  echo "" >&2
  echo "llama-server did not become healthy. Last 100 log lines:" >&2
  tail -n 100 "$LOG_FILE" >&2 || true
  if (( rc == 2 )); then
    die "Server startup timed out after ${STARTUP_TIMEOUT} seconds"
  fi
...
client_config() {
  local adv; adv=$(detect_advertise_ip)
  cat <<CONF
# OpenAI Python SDK — Gemma 4 31B IT Uncensored Heretic Q8_0
...
response = client.chat.completions.create(
    ...
    messages=[
        ...
        {"role": "user", "content": "Explain unified memory in a DGX Spark."},
    ],
    max_tokens=1024,
    temperature=1.0,
    top_p=0.95,
...
# Image — requires server started with vision enabled
image_b64 = base64.b64encode(Path("photo.jpg").read_bytes()).decode("ascii")
response = client.chat.completions.create(
    model="${SERVED_MODEL_NAME}",
    messages=[{
        "role": "user",
...
print(f"Note       : record this result for your own DGX Spark baseline")
PYEOF
}
...
# ─── test-reasoning ───────────────────────────────────────────────────────────
test_reasoning() {
  log "test-reasoning (Gemma thinking may be on or off; expect 37×43=1591) …"
  python3 - <<PYEOF
import urllib.request, json, sys
...
for i in range(2):
    ...
    if shown and not all(c == '?' for c in shown if c.isalpha()):
        ...
        print("WARN: Second multimodal request failed; inspect the server log and current llama.cpp Gemma 4 support")
        sys.exit(1)
PYEOF
}
...
test_tools() {
  ...
  log "test-tools ${mode} (Gemma 4 tool calling — unverified on this llama.cpp build) …"
...
if tc:
    ...
    print("WARN: Tool calling support unverified for Gemma 4 GGUF variant")
    sys.exit(1)
else:
...
case "${1:-help}" in
  ...
  help|--help|-h)
    cat <<HELP
Gemma 4 31B IT Uncensored Heretic Q8_0 single-node controller (llama.cpp)
...
Options:
  ...
  --thinking MODE    Gemma thinking mode: on or off (default: ${THINKING})
  --vision MODE      Vision projector: auto, on, or off (default: ${VISION_MODE})
...
Commands:
  ...
  test-image          Vision: generated 64×64 PNG identification
...
Environment overrides:
  MODEL_QUANT       Quantization variant (default: Q8_0)
  MODEL_FILE        Full filename override
  MMPROJ_FILE       mmproj filename override (default: gemma-4-31B-it-mmproj-BF16.gguf)
  HF_REPO           Hugging Face repo (default: Andu-bit/gemma-4-31B-it-uncensored-heretic-GGUF)
  ...
  THINKING          Gemma thinking mode: on or off (default: ${THINKING})
  VISION_MODE       Vision mode: auto, on, or off (default: ${VISION_MODE})
