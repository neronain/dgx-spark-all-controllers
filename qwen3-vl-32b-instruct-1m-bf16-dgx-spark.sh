#!/usr/bin/env bash
# Qwen3-VL-32B-Instruct-1M BF16 — single DGX Spark controller (llama.cpp)
# Runtime: llama.cpp built from source, SM121a-real (GB10 Blackwell)
# Model: 2 GGUF shards; pass shard 1 to llama.cpp and it auto-loads shard 2
# Vision: mmproj-BF16.gguf required for image/video input
# Instruct model: embedded Jinja2 chat template (--jinja); no reasoning parser
set -Eeuo pipefail

SCRIPT_VERSION="${SCRIPT_VERSION:-3.1.0}"
MODEL_LABEL="${MODEL_LABEL:-Qwen3-VL-32B-Instruct-1M (BF16 GGUF)}"
RUNTIME_LABEL="${RUNTIME_LABEL:-llama.cpp}"
MODEL_FEATURES="${MODEL_FEATURES:-vision · 1M-ctx · tools}"

# ─── Model ────────────────────────────────────────────────────────────────────
HF_REPO="${HF_REPO:-unsloth/Qwen3-VL-32B-Instruct-1M-GGUF}"
MODEL_REVISION="${MODEL_REVISION:-main}"

MODEL_QUANT="BF16"
MODEL_SHARD_1_REL="${MODEL_SHARD_1_REL:-BF16/Qwen3-VL-32B-Instruct-1M-BF16-00001-of-00002.gguf}"
MODEL_SHARD_2_REL="${MODEL_SHARD_2_REL:-BF16/Qwen3-VL-32B-Instruct-1M-BF16-00002-of-00002.gguf}"
MMPROJ_FILE="${MMPROJ_FILE:-mmproj-BF16.gguf}"

SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-qwen3-vl-32b-instruct-1m-bf16}"
...
MODEL_DIR="${MODEL_DIR:-${USER_HOME}/models/qwen3-vl-32b-instruct-1m-bf16}"
...
SHARD_1_SIZE_MIN_BYTES="49000000000"  # remote ≈ 49.9 GB
SHARD_2_SIZE_MIN_BYTES="15000000000"  # remote ≈ 15.7 GB
MMPROJ_SIZE_MIN_BYTES="1000000000"   # remote ≈ 1.2 GB

SHARD_1_SHA256="${SHARD_1_SHA256:-e2e79495aaef68c4fa2274d44c8857518212ee444ae2ec3ae4822efc1b28b200}"
SHARD_2_SHA256="${SHARD_2_SHA256:-181883d356a36e120eff6f82402139cd322402017d8e6ee3782c8c8d223ddf89}"
MMPROJ_SHA256="${MMPROJ_SHA256:-025f953165f604348213d3ec07b4205c140854fe80189804356f21069e944151}"

# ─── Server PID file ──────────────────────────────────────────────────────────
PID_FILE="/tmp/llama-qwen3vl-32b-instruct-1m-bf16.pid"
...
parse_options() {
  ...
  export CTX_SIZE API_HOST API_PORT ADVERTISE_IP ADVERTISE_INTERFACE PARALLEL_SLOTS FLASH_ATTN SPLIT_MODE CACHE_TYPE_K CACHE_TYPE_V
}
...
Files downloaded from ${HF_REPO}:
  ${MODEL_SHARD_1_REL}  ~49.9 GB
  ${MODEL_SHARD_2_REL}  ~15.7 GB
  ${MMPROJ_FILE}        ~1.2 GB

Total download: ~66.7 GB
Recommended free disk space: at least 80 GB
LIST
      return
      ;;
  ...
  if (( force_download == 0 )) \
      && gguf_file_ok "$shard1_path" "$SHARD_1_SIZE_MIN_BYTES" \
      && gguf_file_ok "$shard2_path" "$SHARD_2_SIZE_MIN_BYTES" \
...
verify_files() {
  ...
  verify_one() {
    local label="$1" path="$2" min_bytes="$3" sha="$4"
    [[ -f "$path" ]] || die "${label} not found: ${path}"
    local size magic
    size=$(stat -Lc '%s' "$path" 2>/dev/null || stat -f '%z' "$path")
    (( size >= min_bytes )) || die "${label} too small: ${size} bytes; incomplete download?"
    magic=$(LC_ALL=C head -c 4 "$path" | od -A n -t x1 | tr -d ' \n')
  ...
  local s1 s2 sp
  s1=$(verify_one "model shard 1" "$shard1_path" "$SHARD_1_SIZE_MIN_BYTES" "$SHARD_1_SHA256")
  s2=$(verify_one "model shard 2" "$shard2_path" "$SHARD_2_SIZE_MIN_BYTES" "$SHARD_2_SHA256")
  sp=$(verify_one "mmproj" "$mmproj_path" "$MMPROJ_SIZE_MIN_BYTES" "$MMPROJ_SHA256")
...
start() {
  ...
  if (( CTX_SIZE > 262144 )); then
    log "WARNING: ${CTX_SIZE} context is allowed by the 1M model, but BF16 + very large KV may exceed DGX Spark 128 GB unified memory."
  fi
  ...
  log "Waiting for /health (timeout 300 s) …"
  ...
  die "Server did not start within 300 s. Check logs: $0 logs"
}
...
client_config() {
  local adv; adv=$(detect_advertise_ip)
  cat <<CONF
# OpenAI Python SDK — Qwen3-VL-32B-Instruct-1M BF16 (single DGX Spark)
...
if tc:
    ...
    print("WARN: Tool calling support is not guaranteed for this community fine-tune")
    sys.exit(1)
else:
...
case "${1:-help}" in
  ...
  help|--help|-h)
    cat <<HELP
Qwen3-VL-32B-Instruct-1M BF16 controller (llama.cpp)
...
Environment overrides:
  ...
  HF_REPO           Hugging Face repo (default: unsloth/Qwen3-VL-32B-Instruct-1M-GGUF)
  ...
  CTX_SIZE          Context tokens (default: ${CTX_SIZE}; values above 262144 may exceed Spark memory)
