...
Every controller here was run on real hardware before it was committed — the older ones were
written by hand, the newer ones were generated by LMDS and then served real traffic. That is the whole
point of the collection: students, teammates and customers take a file and run it, without generating
or testing anything themselves.
...
4. `PACKAGE_SHA256SUMS` — regenerate (`shasum -a 256 *.sh *.md *.txt *.py | grep -v PACKAGE_SHA256SUMS`)

`install-canonical.sh` and `verify-all.sh` discover controllers from the directory — nothing to edit there.
...
## What an LMDS-generated controller looks like (0.5.x)

The header is the contract the fleet reads (`lmds recipes --sync` parses it with a regex, never runs it):

| Key | Meaning |
|---|---|
| `MODEL_ID` | Hugging Face repo id — the value `lmds recipes` matches on |
| `MODEL_LABEL` | Defaults to the repo id on generated controllers; the banner shows it verbatim |
| `MODEL_FEATURES` | Overridable (`${MODEL_FEATURES:-…}`); `lmds recipes --publish --features` stamps what was actually measured |
| `VLLM_IMAGE` / `LLAMACPP_IMAGE` | The image that ran, pinned by digest — publish folds in the one `lmds set --image` proved |
| `TOOL_CALL_PARSER` / `REASONING_PARSER` / `ENGINE_ENV` | Same: the values the machine needed, not the plan's first guess |
| `EXTRA_SERVE_ARGS_DEFAULT='…'` | Extra engine flags, **single-quoted** because JSON braces would cut a `${VAR:-…}` short |
| `API_PORT`, `MAX_MODEL_LEN` / `CTX_SIZE`, `GPU_MEMORY_UTILIZATION`, `PARALLEL_SEQS` | Machine values — left at the generator's default on purpose; override with flags or env, never by editing the file |

On the machine that generated it, per-machine overrides live beside the script in `bundle.env` and
`bundle.args`. Those two files are **not** part of this collection: they belong to one machine.
